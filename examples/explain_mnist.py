...
def prepare_batch_for_plotting(a, nrow=4):
    # Normalize
    a /= torch.abs(a).view(num_samples_plot, -1).max(1)[0].view(-1, 1, 1, 1) + 1e-6
    a = (a+1) / 2
...
batch_size = 512
num_samples_plot = min(batch_size, 16)
pattern_max_iter = 10
...
def run_and_plot_rule(rule, ax_, title=None, postprocess=None, pattern=None): 
    ...
    y_hat = y_hat[torch.arange(num_samples_plot), y_hat.max(1)[1]]
    y_hat = y_hat.sum()
